...
local Z= "Monte_FlightSpeed.txt"
local z= "Monte_EggSelectConfig.json"
...
local function Mk(...) h.performanceMode = true pcall(function(...)
        local e=r:FindFirstChild( "Monte_EggESP" )
        if e then
            e:Destroy()
        end
...
local function jM(...)
    ...
    local w=Instance.new ( "TextLabel" )w.Size =UDim2.new ( 1 , -28 , 0 , 24 )w.Position =UDim2.new ( 0 , 14 , 0 , 14 )w.BackgroundTransparency = 1 w.Text = "Monte" w.TextColor3 =Color3.fromRGB ( 245 , 248 , 255 )w.TextSize = 18 w.Font =Enum.Font.GothamBold w.TextXAlignment =Enum.TextXAlignment.Left w.AutoLocalize = false w.Parent =r
    ...
    local k=Instance.new ( "TextLabel" )k.Size =UDim2.new ( 0 , 50 , 0 , 24 )k.Position =UDim2.new ( 1 , -64 , 0 , 14 )k.BackgroundTransparency = 1 k.Text = "0%" k.TextColor3 =Color3.fromRGB ( 0 , 255 , 100 )k.TextSize = 14 k.Font =Enum.Font.GothamBold k.TextXAlignment =Enum.TextXAlignment.Right k.AutoLocalize = false k.Parent =r
    ...
    local H=Instance.new ( "UIGradient" ,V)H.Color =ColorSequence.new ({ColorSequenceKeypoint.new ( 0 ,Color3.fromRGB ( 0 , 185 , 255 )),ColorSequenceKeypoint.new ( 1 ,Color3.fromRGB ( 0 , 255 , 100 ))})
    local t=Instance.new ( "TextLabel" )t.Size =UDim2.new ( 1 , -28 , 0 , 16 )t.Position =UDim2.new ( 0 , 14 , 0 , 94 )t.BackgroundTransparency = 1 t.Text = "Initializing Monte..." t.TextColor3 =Color3.fromRGB ( 130 , 140 , 165 )t.TextSize = 11 t.Font =Enum.Font.Gotham t.TextXAlignment =Enum.TextXAlignment.Left t.AutoLocalize = false t.Parent =r task.spawn (function(...)
        for y= 1 , 100 , 1 do
            if not e.Parent then
                break
            end
...
local function aM(...) h.alive = false pcall(Ik)pcall(Ak)pcall(function(...) y:Set3dRenderingEnabled( true )
        ...
        local y=r:FindFirstChild( "Monte_EggESP" )
        if y then
            y:Destroy()
        end
    end
...
local function oM(...)
    ...
    local function j(e,...)
        ...
        if not y then
            pcall(function(...)
                (game:GetService( "StarterGui" )):SetCore( "SendNotification" ,{[ "Title" ]=tostring(e.Title or "Monte" ),[ "Text" ]=tostring(e.Content or "" );
                [ "Duration" ]= 3 })
            end
            )
        end
    ...
    if r then
        pcall(function(...)
            ...
            if e then
                r.Notify =function(r,y,...)
                    ...
                    if not u then
                        pcall(function(...)
                            (game:GetService( "StarterGui" )):SetCore( "SendNotification" ,{[ "Title" ]=tostring(y and y.Title or "Monte" );
        ...
        local p=r:CreateWindow({[ "Title" ]= "Monte" ;
        ...
        [ "Color" ]=Color3.fromRGB ( 0 , 255 , 100 ),[ "Border" ]= true })
        local J= 44
        local K= false
        local c= false
        ...
        [ "Icon" ]= "solar:trash-bin-trash-bold" ,[ "Callback" ]=function(...) aM()
            ...
            while h.alive do
                pcall(function(...)
                    ...
                    local u=Wk(Xk)
                    if B then
                        local e=Color3.fromRGB ( 0 , 255 , 100 )
    ...
    h.gui =k
    local V=Color3.fromRGB ( 8 , 10 , 8 )
    local H=Color3.fromRGB ( 12 , 16 , 12 )
    local t=Color3.fromRGB ( 16 , 22 , 16 )
    local s=Color3.fromRGB ( 22 , 30 , 22 )
    ...
    local l=Instance.new ( "TextLabel" )l.Size =UDim2.new ( 1 , -90 , 0 , 22 )l.Position =UDim2.new ( 0 , 12 , 0 , 6 )l.BackgroundTransparency = 1 l.Text = "Monte Hub" l.TextColor3 =B l.TextSize = 14 l.Font =Enum.Font.GothamBold l.TextXAlignment =Enum.TextXAlignment.Left l.AutoLocalize = false l.Parent =U
    local D=Instance.new ( "TextLabel" )D.Size =UDim2.new ( 1 , -90 , 0 , 14 )D.Position =UDim2.new ( 0 , 12 , 0 , 26 )D.BackgroundTransparency = 1 D.Text = "Steal an Egg v42.64" D.TextColor3 =Color3.fromRGB ( 0 , 255 , 100 )D.TextSize = 11 D.Font =Enum.Font.Gotham D.TextXAlignment =Enum.TextXAlignment.Left D.AutoLocalize = false D.Parent =U
    ...
    local S= false
    local Z=nil
    local z=nil Z=b( "Auto Steal (Tween)" , "Fly to steal eggs & auto stash into backpack" ,h.pureTweenFarm ,Color3.fromRGB ( 0 , 255 , 100 ), 11 ,function(e,...)
        if S then
            return
        end
        if e then
    ...
    local T=Instance.new ( "TextLabel" )T.Size =UDim2.new ( 0 , 70 , 0 , 24 )T.Position =UDim2.new ( 1 , -80 , 0.5 , -12 )T.BackgroundColor3 =V T.Text =string.format ( "%d Studs/s" ,h.glideSpeed or 600 )T.TextColor3 =Color3.fromRGB ( 0 , 255 , 100 )T.TextSize = 11 T.Font =Enum.Font.GothamBold T.AutoLocalize = false T.Parent =F;
...
H( "[+] Initializing Monte Hub v1.0 (Steal an Egg Edition)..." )oM()task.spawn (function(...) task.wait ( 0.5 )A4()b4( true )C4()
    if o.Character then
        z4(o.Character )
    end
